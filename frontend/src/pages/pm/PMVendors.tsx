...
const PMVendors = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        await createVendor({ id: "", name: form.name, email: form.email, phone: form.phone, specialty: form.specialty });
        toast({ title: "Vendor added" });
      }
      setDialogOpen(false);
    } catch (err: unknown) {
      const msg = err && typeof err === "object" && "message" in err ? String((err as { message: string }).message) : "Failed to save vendor";
      toast({ title: "Failed to save vendor", description: msg, variant: "destructive" });
    }
  };
  ...
  const handleDelete = async (vendorId: string) => {
    ...
    try {
      ...
      const msg = err && typeof err === "object" && "message" in err ? String((err as { message: string }).message) : "Failed to delete vendor";
      toast({ title: "Failed to delete vendor", description: msg, variant: "destructive" });
    }
  };

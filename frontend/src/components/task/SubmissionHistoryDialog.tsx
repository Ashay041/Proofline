...
import { CheckCircle2, ImageIcon, MapPin, ExternalLink } from "lucide-react";
import type { TaskSubmission } from "@backend/types";
            ...
            {sub.geoLat != null && sub.geoLng != null && (
              <div className="flex items-center justify-between gap-2 rounded border bg-muted/30 p-2">
                <span className="text-xs text-muted-foreground flex items-center gap-1">
                  ...
                  {sub.geoLat.toFixed(5)}, {sub.geoLng.toFixed(5)}
                </span>
                <a
                  href={`https://www.google.com/maps?q=${sub.geoLat},${sub.geoLng}`}
                  target="_blank"

...
import { type Task, type ReworkItem } from "@/types";
import { useAppState } from "@/context/TaskContext";
import {
  CheckCircle2, RotateCcw, Camera, AlertTriangle, Image as ImageIcon,
  Clock, CalendarDays, Info, History, Phone, ChevronLeft, ChevronRight
} from "lucide-react";
...
const PMTaskReviewDialog = ({ task, open, onOpenChange }: PMTaskReviewDialogProps) => {
  ...
  const handleApprove = () => {
    updateTask(task.id, { status: "Approved", reworkNote: undefined, reworkItems: undefined });
    toast({ title: "Task approved", description: "The vendor has been notified." });
    onOpenChange(false);
  };
  ...
  const handleRequestRework = () => {
    ...
    doRework(task.id, reworkItems, reworkNote.trim() || undefined);
        ...
        <div className="space-y-4 py-2">
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-muted-foreground">{checkedCount}/{task.checklist.length}</span>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {isResubmission && (
                <>
                  ...
                  {task.checklist.some((item) => !lastReworkIds.has(item.id)) && (
          ...
          <section>
            <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1">
              <Camera className="h-3.5 w-3.5" /> Proof Photos ({task.photos.length})
            </h3>
    ...
    <Dialog open={selectedPhotoIndex !== null} onOpenChange={(open) => { if (!open) setSelectedPhotoIndex(null); }}>
      <DialogContent className="max-w-3xl bg-black/95 border-none p-2 sm:p-4">
        <DialogHeader className="sr-only">
          <DialogTitle>Photo {selectedPhotoIndex !== null ? selectedPhotoIndex + 1 : ""} of {task.photos.length}</DialogTitle>
          <DialogDescription>Viewing proof photo</DialogDescription>
        </DialogHeader>
        {selectedPhotoIndex !== null && (
          <div className="relative flex items-center justify-center min-h-[50vh]">
            <img
              src={task.photos[selectedPhotoIndex]}
            ...
            {task.photos.length > 1 && (
              <>
                <button
                  onClick={() => setSelectedPhotoIndex((selectedPhotoIndex - 1 + task.photos.length) % task.photos.length)}
                ...
                <button
                  onClick={() => setSelectedPhotoIndex((selectedPhotoIndex + 1) % task.photos.length)}
            ...
            <span className="absolute bottom-2 left-1/2 -translate-x-1/2 text-xs text-white/80 bg-black/50 px-2 py-1 rounded">
              Photo {selectedPhotoIndex + 1} of {task.photos.length}
            </span>
          </div>
        )}
      </DialogContent>

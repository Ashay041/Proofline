...
/** Get the most recent rework items from submission history (scan backwards for the first entry with reworkItems). */
export const getLastReworkItems = (task: Task): ReworkItem[] => {
  const history = task.submissionHistory ?? [];
  for (let i = history.length - 1; i >= 0; i--) {
    if (history[i].reworkItems && history[i].reworkItems!.length > 0) {
      return history[i].reworkItems!;
    }
  }
  return [];
};

/** Get the most recent rework note from submission history (scan backwards for the first entry with a reworkNote). */
export const getLastReworkNote = (task: Task): string | undefined => {
  const history = task.submissionHistory ?? [];
  for (let i = history.length - 1; i >= 0; i--) {
    if (history[i].reworkNote) return history[i].reworkNote;
  }
  return undefined;
};
